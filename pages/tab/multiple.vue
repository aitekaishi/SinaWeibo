			...
			<view v-for="(item, index) in indexList" :key="index">
				<view class="title">
					<view class="title-left">
						<view class="header">
							...
							<view
								:style="{ opacity: item.mblog.user.verified ? 1 : 0 }"
								:class="[item.mblog.user.verified_type == 3 ? 'blue-vip' : item.mblog.user.verified_type_ext == 1 ? 'red-vip' : 'yellow-vip']"
							></view>
						</view>
						<view class="name">
					...
					<view class="title-right">+关注</view>
				</view>
				...
				<view class="image-video">
					...
					<view v-if="item.mblog.page_info" class="video">
						<view @click="playVideo(item.mblog.page_info.page_url)" v-if="item.mblog.page_info.media_info">
				...
				<view v-if="type" class="operate">
					<view class="operate-left">
						<view>
							<image src="/static/index/relay.svg" mode="widthFix"></image>
							<text>{{ item.mblog.reposts_count }}</text>
						</view>
						<view>
							<image src="/static/index/comment.svg" mode="widthFix"></image>
							<text>{{ item.mblog.comments_count }}</text>
						</view>
						<view>
							<image src="/static/index/admire.svg" mode="widthFix"></image>
							<text>{{ item.mblog.attitudes_count }}</text>
						</view>
					</view>
